    ...
    AccessControlUpgradeable
{
    ...
    // block existing epochMmrRoots in form of Header eponch mapping mmrRoot => bool
    ...
    // checking sender is member of Admin 
    modifier onlyAdmin() {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "sender doesn't have admin role");
        _;
    }

    // checking sender is member of  Relayer Role 
    modifier onlyRelayers() {
        require(hasRole(RELAYER_ROLE, msg.sender), "sender doesn't have relayer role");
        _;
    }

    //only admin pause light client
    function adminPauseLightClient() external onlyAdmin {
        _pause();
    }

    //only admin unpause light client
    ...
    //admin can remove relayer address
    ...
    //Used to submit checkpoint information to contract
    ...
    //Used to get latest checkpoint data from contract
    ...
    //Checking checkpoint is valid or not
