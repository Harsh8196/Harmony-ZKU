...
void createMerkletree_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void createMerkletree_3_run(uint ctx_index,Circom_CalcWit* ctx);
Circom_TemplateFunction _functionTable[4] = { 
MiMCFeistel_0_run,
MiMCSponge_1_run,
MiMCSponge_2_run,
createMerkletree_3_run };
uint get_main_input_signal_start() {return 2;}

uint get_main_input_signal_no() {return 8;}

uint get_total_signal_no() {return 19503;}
...
ctx->componentMemory[coffset].templateName = "createMerkletree";
ctx->componentMemory[coffset].signalStart = soffset;
...
for (uint i = 0; i < 8; i++) {
std::string new_cmp_name = "mimc"+ctx->generate_position_array(aux_dimensions, 1, i);
mySubcomponents[aux_create+i] = aux_cmp_num;
...
uint csoffset = mySignalStart+7112;
uint aux_dimensions[1] = {7};
for (uint i = 0; i < 7; i++) {
std::string new_cmp_name = "mimcN"+ctx->generate_position_array(aux_dimensions, 1, i);
mySubcomponents[aux_create+i] = aux_cmp_num;
...
Fr_lt(&expaux[0],&lvar[2],&circuitConstants[17]); // line circom 13
while(Fr_isTrue(&expaux[0])){
{
...
Fr_gt(&expaux[0],&lvar[2],&circuitConstants[1]); // line circom 27
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &lvar[4];
// load src
...
Fr_lt(&expaux[0],&lvar[4],&lvar[2]); // line circom 28
while(Fr_isTrue(&expaux[0])){
Fr_mod(&expaux[1],&lvar[4],&circuitConstants[5]); // line circom 29
Fr_eq(&expaux[0],&expaux[1],&circuitConstants[3]); // line circom 29
if(Fr_isTrue(&expaux[0])){
Fr_sub(&expaux[0],&lvar[0],&circuitConstants[17]); // line circom 31
{
uint cmp_index_ref = ((1 * Fr_toInt(&expaux[0])) + 8);
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
Fr_add(&expaux[1],&lvar[3],&lvar[4]); // line circom 31
Fr_sub(&expaux[0],&expaux[1],&circuitConstants[3]); // line circom 31
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * Fr_toInt(&expaux[0])) + 9)]);
}
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
...
Fr_sub(&expaux[0],&lvar[0],&circuitConstants[17]); // line circom 32
{
uint cmp_index_ref = ((1 * Fr_toInt(&expaux[0])) + 8);
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 2];
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * (Fr_toInt(&lvar[3]) + Fr_toInt(&lvar[4]))) + 9)]);
}
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
MiMCSponge_2_run(mySubcomponents[cmp_index_ref],ctx);
...
Fr_sub(&expaux[0],&lvar[0],&circuitConstants[17]); // line circom 33
{
uint cmp_index_ref = ((1 * Fr_toInt(&expaux[0])) + 8);
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 3];
...
{
PFrElement aux_dest = &signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[0])) + 9)];
// load src
Fr_sub(&expaux[1],&lvar[0],&circuitConstants[17]); // line circom 34
// end load src
Fr_copy(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&expaux[1])) + 8)]].signalStart + 0]);
}
{
PFrElement aux_dest = &lvar[0];
// load src
Fr_add(&expaux[0],&lvar[0],&circuitConstants[3]); // line circom 35
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
}
...
PFrElement aux_dest = &lvar[4];
// load src
Fr_add(&expaux[0],&lvar[4],&circuitConstants[3]); // line circom 28
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[4],&lvar[2]); // line circom 28
...
PFrElement aux_dest = &lvar[2];
// load src
Fr_div(&expaux[0],&lvar[2],&circuitConstants[5]); // line circom 40
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_gt(&expaux[0],&lvar[2],&circuitConstants[1]); // line circom 27
...
{
char* temp = Fr_element2str(&signalValues[mySignalStart + 10]);
printf("%s\n",temp);
delete [] temp;
}
{
char* temp = Fr_element2str(&signalValues[mySignalStart + 11]);
printf("%s\n",temp);
delete [] temp;
}
{
...
{
char* temp = Fr_element2str(&signalValues[mySignalStart + 13]);
printf("%s\n",temp);
delete [] temp;
}
{
...
createMerkletree_3_create(1,0,ctx,"main",0);
createMerkletree_3_run(0,ctx);
}
