...
void init_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void init_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 5317;}

uint get_number_of_components() {return 10;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 3964;}
...
ctx->componentMemory[coffset].templateName = "init";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 3;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "mimc_card1";
mySubcomponents[aux_create+i] = aux_cmp_num;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "mimc_card2";
mySubcomponents[aux_create+i] = aux_cmp_num;
MiMCSponge_1_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 1770 ;
aux_cmp_num += 3;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "mimc_comitment";
mySubcomponents[aux_create+i] = aux_cmp_num;
MiMCSponge_1_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 1770 ;
aux_cmp_num += 3;
